...
void ChooseSemesterStudentScene::drawChooseSemester(sf::RenderWindow& window, Static* a)
{
...
void ChooseSemesterStudentScene::renderChooseSemester(sf::Event event, Scene* scene, sf::RenderWindow& window)
{
    ...
    if (scene->a->currentState == programState::ChooseSemesterStudent && preButton.getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y)))
    {
    ...
    for (int i = 0; i < numSemester; i++) {
        if (scene->a->currentState == programState::ChooseSemesterStudent && buttons[i].getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
...
ChooseSemesterStudentScene::~ChooseSemesterStudentScene()
{
    delete[] buttons;
    delete[] labels;
    delete year;
