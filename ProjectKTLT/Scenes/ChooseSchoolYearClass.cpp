#include "ChooseSchoolYearClass.h"
#include "../SchoolYear.h"
#include <fstream>
#include "Scene.h"

ChooseSchoolYearClassScene::ChooseSchoolYearClassScene(Static* a)
{
...
void ChooseSchoolYearClassScene::drawChooseSchoolYearClassScene(sf::RenderWindow& window, Static* a)
{
...
void ChooseSchoolYearClassScene::renderChooseSchoolYearClassScene(sf::Event event, Scene* scene, sf::RenderWindow& window)
{
    sf::Vector2i mousePos = sf::Mouse::getPosition(window);
    if (scene->a->currentState == programState::ChooseSchoolYearClass && preButton.getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y)))
    {
    ...
    for (int i = 0; i < numSchoolYears; i++) {
        if (scene->a->currentState == programState::ChooseSchoolYearClass && buttons[i].getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
    ...
    if (event.type == sf::Event::MouseButtonPressed)
    {
        if (event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            {
                delete scene->chooseschoolyearclass;
                scene->chooseschoolyearclass = nullptr;
...
ChooseSchoolYearClassScene::~ChooseSchoolYearClassScene()
{
    delete[] buttons;
    delete[] labels;
