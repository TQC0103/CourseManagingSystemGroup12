...
SignInAsStudentScene::SignInAsStudentScene()
{
	...
	createText(passwordStudentText, a.fontN, sf::Color::White, "", 60, a.width / 2.0f, 475.0f);
...
void SignInAsStudentScene::drawSignInAsStudent(sf::RenderWindow& win)
{
	...
	if (usernameInputEnable)
	{
		...
		sf::Vector2f cursorPos = usernameStudentText.findCharacterPos(usernameStudentInput.size());

		// Draw cursor as a vertical line
		sf::RectangleShape cursor(sf::Vector2f(1.0f, usernameStudentText.getCharacterSize()));
		setOriginRecToMiddle(cursor);
